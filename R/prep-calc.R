...
#'
#' @param x A `data.frame` (or `tr_data`) object containing data.
#' @param feats Character. A vector of variables names.
#' @param response `character(1)`. The column name to use as the
#'   response/grouping variable. Expected to be a factor with 2 levels.
#' @param paired Logical. Whether the data are paired or not.
#' @param binary Logical. Whether the `response` is expected binary.
#' @return A list of the following:
#'   \item{data}{The "prepared" new training data set.}
#'   \item{feats}{The aptamers to use; categorical (factor) variables removed.}
#'   \item{pos_idx}{Numeric row indices of the positive class.}
#' @importFrom tibble tibble
#' @noRd
prep_calc <- function(x, feats, response, paired = FALSE, binary = TRUE) {
  ...
  pos_idx       <- which(x[[response]] == disease_class)
  ...
  if ( length(pos_idx) == nrow(x) ) {
    stop("Response column is bad. All disease.", call. = FALSE)
  }

  # paired analyses ----
  if ( paired ) {
    tokens <- c("SubjectID", "SubjectId", "subjectId", "subject.id",
                "subjectID", "PID", "pid", "Pid")
    if ( any( tokens %in% names(x)) ) {
      tokens <- tokens[ tokens %in% names(x) ]
      for ( check in tokens ) {
        if ( any(x[ pos_idx, check] != x[-pos_idx, check]) ) {
          warning(
            "Subject IDs may not match in paired analysis ... please check.",
            call. = FALSE
          )
          signal_info("Disease class rows:", value(pos_idx))
          flag <- tibble(disease = x[pos_idx, check],
                         control = x[-pos_idx, check])
  ...
  good_fts <- discard_it(feats, vapply(x[, feats], is.factor, NA))
  ...
  list(data    = x[, good_fts, drop = FALSE],
       feats   = good_fts,
       pos_idx = pos_idx)
}
