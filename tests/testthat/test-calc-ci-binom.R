...
test_that("a single value returns correct binomial limits", {
  a <- calc_ci_binom(0.8, 20)     # default 95% ci
  b <- calc_ci_binom(0.45, 20)    # default 95% ci
...
test_that("the vectorized form also works", {
  ci <- calc_ci_binom(seq(0, 1, length.out = 9L), 20)
  expect_equal(
    ci,
